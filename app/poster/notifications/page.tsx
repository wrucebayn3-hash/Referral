    ...
    <DashboardLayout role="poster" userName={session.user.name ?? undefined} userRole={session.user.role}>
      <div className="max-w-2xl space-y-6">
        ...
        {notifications.length === 0 ? (
          <EmptyState icon={Bell} title="No notifications yet" description="You'll see updates about referral requests here." />
        ) : (
          <div className="space-y-2">
            {notifications.map((n) => {
              const Icon = typeIcons[n.type] || Info
              return (
                <div key={n.id} className={cn('card flex items-start gap-3 p-4', !n.read && 'border-l-4 border-l-coral bg-pale-blue/30')}>
                  <div className="w-8 h-8 rounded-sm bg-soft-stone flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {n.link && <Link href={n.link} className="text-micro text-action-blue hover:underline mt-1 block">View details →</Link>}
                  </div>
